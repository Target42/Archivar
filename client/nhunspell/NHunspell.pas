...
type
  ...
  private
    ...
    FSpellDictionay: TNHSpellDictionary;
    FHyphenDictionary: TNHHyphenDictionary;
  ...
  public
    ...
    function SelectSpellDictionary(index: integer) : boolean;
    function SelectHyphenDictionary(index: integer) : boolean;
    ...
    property SpellDictionary : TNHSpellDictionary read FSpellDictionay;
    property HyphenDictionary: TNHHyphenDictionary read FHyphenDictionary;
  end;
...
function TNHunspell.SelectHyphenDictionary(index: integer): boolean;
begin
  if Assigned(FHyphenDictionary) then
    FHyphenDictionary.Active := false;

  FHyphenDictionary := NIL;
  if (index >= 0 ) and ( index < FHyphenDictionaries.Count) then begin
    FHyphenDictionary := TNHHyphenDictionary(FHyphenDictionaries.Objects[index]);
    FHyphenDictionary.Active := true;
  end;
  Result := Assigned(FHyphenDictionary);
end;

function TNHunspell.SelectSpellDictionary(index: integer): boolean;
begin
  if Assigned(FSpellDictionay) then
    FSpellDictionay.Active := false;

  FSpellDictionay := NIL;
  if (index >= 0 ) and ( index < FSpellDictionaries.Count) then begin
    FSpellDictionay := TNHSpellDictionary(FSpellDictionaries.Objects[index]);
    FSpellDictionay.Active := true;
  end;
  Result := Assigned(FSpellDictionay);
end;
